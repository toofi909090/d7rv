...
class GifShopSystem {
  ...
  async safeReply(interaction, options) {
    try {
      ...
      const now = Date.now();
      const interactionTime = interaction.createdTimestamp;
      const maxAge = 14 * 60 * 1000;
      
      if (now - interactionTime > maxAge) {
        console.warn('Interaction expired, cannot respond');
        return null;
      }
  ...
  async safeUpdate(interaction, options) {
    try {
      ...
      if (now - interactionTime > maxAge) {
        console.warn('Interaction expired, cannot update');
        return null;
      }
  ...
  async showGifShop(interaction, page = 0) {
    try {
      ...
      if (currentPageGifs.length === 0) {
        const response = await this.safeReply(interaction, {
  ...
  async handleButtonInteraction(interaction) {
    ...
    try {
      ...
      if (!this.isValidInteraction(interaction)) {
        console.warn('Invalid interaction, skipping');
        return;
      }
        ...
        setTimeout(async () => {
          try {
            ...
            const fakeInteraction = {
              ...
              replied: false,
              deferred: false,
              customId: 'bag_view_gifs'
            };
            await this.showGifsInBag(fakeInteraction);
          } catch (error) {
